...
# Ollama setup - default URL for local installation
OLLAMA_URL = os.getenv("OLLAMA_URL", "http://localhost:11434/api")
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "gemma3:12b")

from pathlib import Path

# Resolve the full path to the current script
...
# Optional: create output dir if missing
...
def extract_info_from_text(text):
    """Send prompt with text to Ollama and return response content"""
...
def batch_extract_entities(input_folder, output_folder):
    ...
    for file_path in text_files:
        ...
        try:
            ...
            logging.info(f"Extracting entities from: {filename}")
            ...
            if not all_matches:
                logging.warning(f"No entities found in: {filename}")
                continue
